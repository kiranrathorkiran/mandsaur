...
function Section({ title, imageSrc, altText, description }) {
  return (
    <div className=" bg-slate-900 p-0 m-0 w-screen h-full   md:flex-row  flex flex-col items-center text-center ">
      ...
      <img
        className="w-screen  px-5 py-16 max-w-[600px] h-auto object-cover rounded-md"
        src={imageSrc}
        alt={altText}
      />
      <div>
  ...
  return (
    <div className="bg-slate-900 p-0 m-0 w-screen h-full flex flex-col items-center text-center ">
      ...
      <iframe
        ...
        className="w-screen h-64 sm:h-80 md:h-96 max-w-[600px] border-none  gap-6 px-5 py-10 rounded-md"
        allowFullScreen
      ></iframe>
    </div>
  );
